...
{
    $TCA .= "\n\t\t\$markerArray['###" . $markerNames[$key."Marker"] . "###'] = \$this->cObj->data['".$value."'];";
    $markers .= "\n\t###" . $markerNames[$key."Marker"] . "###";
    $TCAforTables .= "\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t,$value;LLL:EXT:$extNameU/locallang_db.xml:tt_content.$piName.$value";
    $locallang .= "\n\t\t\t<label index=\"tt_content.$piName.$key\"> ". $localangLable[$key] ." </label>";
}
...
//class.tx_goteaser_piHomeWhiteTeaser
// echo "<br />";
// $newData = str_replace("pi1","piFanTeaserEBIKE",$data);
// $newData2 = str_replace("goteaser","goslider",$newData);
// $newData3 = str_replace("go_teaser","go_slider",$newData2);
...
// mkdir($piName, 0700);
// echo count($getArray);
// text_1;LLL:EXT:go_teaser/locallang_db.xml:tt_content.piWhiteteaser2.link

//Read Before Creating
// echo "<pre>";
// print_r(readBeforeAfter("protected function doTemplateParsing() {",$data));
// echo "</pre>";
